      ...
      <div ref={ref} className="flex md:hidden flex-col items-center gap-3">
        {
          skills.map((skill: Skill, idx: number) => (
            <SkillCard key={skill.priority} skill={skill} minWidth="min-w-[13rem]" textSize="md" inView={inView} startingOpacity={"opacity-0"} additionalStyles={{ animationDelay: (idx * 200) + "ms" }} />
          ))
        }
      </div>
    )
